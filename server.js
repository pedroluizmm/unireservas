...
function validaLocalizacao(loc) {
  if (!loc) return true;
  return ['interna', 'externa'].includes(loc.toLowerCase());
}
...
app.post('/api/restaurantes', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: restauranteId, nome, endereco });
  } catch (err) {
...
app.get('/api/mesas', async (req, res) => {
  ...
  const sql = restauranteId ? 'SELECT * FROM MESA WHERE restaurante_id = ?' : 'SELECT * FROM MESA';
  try {
    const rows = await query(sql, restauranteId ? [restauranteId] : []);
    res.json(rows);
  } catch (err) {
    console.error(err);
...
app.post('/api/mesas', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: result.insertId, restauranteId, capacidade, localizacao });
  } catch (err) {
    console.error(err);
...
app.put('/api/mesas/:id', async (req, res) => {
  ...
  try {
    ...
    res.json({ id, capacidade, localizacao });
  } catch (err) {
    console.error(err);
...
app.post('/api/verificar-disponibilidade', async (req, res) => {
  ...
  if (!restauranteId || !horario || !numPessoas) {
    return res.status(400).json({ message: 'Dados incompletos' });
  }
  if (!validaLocalizacao(localizacao)) {
    return res.status(400).json({ message: 'Localização inválida' });
...
app.post('/api/criar-reserva', async (req, res) => {
  ...
  if (!clienteId || !restauranteId || !horario || !numPessoas || !valorTotal) {
...
app.put('/api/reservas/:id', async (req, res) => {
  const { horario, numPessoas, localizacao, statusPagamento } = req.body;
  const id = req.params.id;
  if (!horario && !numPessoas && !localizacao && !statusPagamento) {
  ...
  try {
    await query(
      'UPDATE RESERVA SET horario = COALESCE(?, horario), num_pessoas = COALESCE(?, num_pessoas), preferencia_localizacao = COALESCE(?, preferencia_localizacao), status_pagamento = COALESCE(?, status_pagamento) WHERE id_reserva = ?',
      [horario, numPessoas, localizacao, statusPagamento, id]
    );
    res.json({ id, horario, numPessoas, localizacao, statusPagamento });
  } catch (err) {
    console.error(err);
